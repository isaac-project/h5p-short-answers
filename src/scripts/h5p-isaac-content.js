...
export default class ISAACContent {
  ...
  constructor(task, passage, questions, contentID, backend, prev) {
    ...
    if (passage.trim() !== '') {
      ...
      const tag = `<span id='${contentID}_mark_$1' class='h5p-isaac-highlight h5p-isaac-highlight-hidden'>$2</span>`;
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      questionWrapper.firstElementChild.setAttribute("id", `${contentID}_prompt_${i + 1}`);
      ...
      userInput.addEventListener("keydown", function (e) {
        if (e.key === 'Enter') {
          ...
          listener.handleEvent(document.getElementById(`${contentID}_input_${i}`).textContent);
